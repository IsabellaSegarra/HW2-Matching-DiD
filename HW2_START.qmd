...
*Response:* in order to assess the risk of spillovers, especially considering the affect of turbines on neighboring precincts, the authors redefined treatment in terms of distance to turbines. 
...
*Response:* This dataset is ideal for matching because it has many covariates and control observations. In this way, you can take each treated unit and look across another group for the closest match . 
...
*Response:* In order to conduct a matching analysis, you need to use panel data with many covariates and observaitons. 
...
*Response:*The purpose of a love plot is to assess which variables are potentially problematic in this analysis because they have a large SMD. The love plot shows that population density and percent university degree show a large SMD. 
...
*Response:* Without the robust=TRUE argument, the SE is 0.031 and with the argument, the SE is 0.04. This changes the standard error because outliers in the dataset are not changing the averages. 
...
model2_ff <- fixest::feols(perc_lib ~ proposed_turbine, data = panel_data)
...
*Response (4-6 sentences):* This model is attempting to show the relationship between proposed wind turbines in precincts within the Ontario region and its impact on local governance vote or whether it influences the Liberal party vote. The model predicts that wind turbines will lead to a 10% decline in liberal vote, showing a significant "p-value" or it is unlikely this relationship is happening by chance. The model shows that for a precinct, there is an average percent of 36% liberal that will drop to 26% after proposed turbines. Clustering by precinct id is important because it accounts for the natural variation across precincts. 
...
model3_ff <- fixest::feols(perc_lib ~ operational_turbine, data = panel_data)
...
*Response:* For precincts with operational turbines, there is a 7% decline in Liberal party vote.  
