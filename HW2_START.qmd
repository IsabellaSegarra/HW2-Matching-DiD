...
*Response (2-4 sentences):* These variables can influence the treatment because larger precints are associated with more urban regions and therefore, more Liberal preferences. In addition, universty degrees are also associated with more Liberal preferences. 
...
*Response:* The covariate with the largest `Std. Mean Diff.` or SMD is `log_home_val_07` and the covariate with the smallest is 
...
reg_match <- matchit(change_liberal ~ proposed_turbine_3km,
     # Treatment_indicator ~  Pre_treatment_covariates
  data = match_data
)
